...
export default function Home() {
  ...
  return (
    <div>
      <div>
      <h1 className='mb-6 text-3xl font-bold tracking-tight text-white'>Good afternoon</h1>
      <div className='grid items-center justify-center grid-cols-3 mb-6 transition-all gap-x-6 gap-y-4'>
        <div className='relative flex items-center rounded gap-x-4 bg-dortbox group hover:bg-dortboxact'>
          ...
          <button className='absolute flex items-center justify-center w-12 h-12 transition-opacity rounded-full shadow-2xl opacity-0 bg-primary right-2 group-hover:opacity-100 group-active:opacity-100'>
            <Icon name="play"/>
          </button>
        </div>
        ...
        <PlayBox title={'Made for you'}/>
        <PlayBox see />
        </div>
        <div className='grid grid-cols-5 mb-8 gap-x-6'>
                {mixer.map(item => (
                    <NavLink key={item.id} to="/" className={'bg-footer rounded p-4 transition-all hover:bg-menubg group'}>
                        <div className='relative '> 
                          <img className='inset-0 object-cover w-auto h-auto mb-4' src={item.image} />
                          <button className='absolute flex items-center justify-center w-12 h-12 transition-opacity rounded-full shadow-2xl opacity-0 bg-primary right-2 bottom-2 group-hover:opacity-100 group-active:opacity-100'>
                            <Icon name="play"/>
                          </button>
                        </div>
        ...
        <div className='grid grid-cols-5 mb-8 gap-x-6'>
                {singer.map(item => (
                    <NavLink key={item.id} to="/" className={'bg-footer rounded p-4 transition-all hover:bg-menubg group'}>
                        <div className='relative '> 
                          <img className='inset-0 object-cover w-auto h-auto mb-4 rounded-full' src={item.image} />
                          <button className='absolute flex items-center justify-center w-12 h-12 transition-opacity rounded-full shadow-2xl opacity-0 bg-primary right-2 bottom-2 group-hover:opacity-100 group-active:opacity-100'>
                            <Icon name="play"/>
                          </button>
                        </div>
